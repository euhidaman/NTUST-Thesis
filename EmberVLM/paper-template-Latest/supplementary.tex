...
Stage~5 knowledge consolidation was \emph{not} applied for any result in the main text.
...
\noindent\textbf{Time(s) in main text Table~5} reports total wall-clock inference over the complete evaluation set on an AMD Ryzen~3 4100 (3.80\,GHz, 32\,GB RAM, no GPU) — the intended deployment hardware. Per-sample latency\,=\,Time(s)\,$\div$\,\#samples. All baseline models were evaluated on identical hardware for a fair comparison.
...
Table~5 of the main text. ``EmberVLM (No\,Mem.)'' is used explicitly for the
...
%    SmolVLM2=0.433, EmberVLM=0.279 (calculated from 17.2%)
%
...
runs at ${\approx}180\times$ lower latency on CPU-only hardware. The coherence gain from
episodic memory (14.8\%\,$\rightarrow$\,17.2\%) shows the memory module partially
...
% Note: Supplementary uses complete POPE dataset (9,000 samples) vs. 100-sample subset in main paper
\begin{table}[ht]
\centering
\caption{POPE hallucination rates by split, with and without VA Refiner (\%), evaluated on the complete 9,000-sample dataset. Lower is better. $\Delta$ = absolute reduction.}
\label{tab:pope}
\scriptsize
...
EmberVLM           & 29.00 & 22.00 & 21.20 & 15.00 & 6.70 & 6.13 & 29.00 & \textbf{22.00} & \textbf{$-$7.00} \\
\bottomrule
\end{tabular}
\end{table}
...
evaluated on the complete 9,000-sample dataset (cf.\ 100-sample subset in main paper).
...
detection that generalises across both generative and grounded outputs. Evaluation
on the full 9,000-sample POPE dataset (vs.\ the 100-sample subset reported in the
main text) confirms the VA Refiner's robustness at scale.
...
consistent with Fig.~2 of the main text, which shows SmolLM-135M mid-decoder MLP

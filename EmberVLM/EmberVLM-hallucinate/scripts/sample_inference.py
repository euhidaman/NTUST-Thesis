...
def load_model_from_checkpoint(checkpoint_path: str, device: str = 'cuda') -> Tuple[EmberVLM, AutoTokenizer]:
    ...
    # Resize embeddings BEFORE loading weights if config has resized vocab
    if hasattr(config, 'language_vocab_size'):
        target_vocab = config.language_vocab_size
        current_vocab = None
        ...
        if current_vocab and current_vocab != target_vocab:
            logger.info(f"Resizing embeddings from {current_vocab} to {target_vocab} before loading weights")
            if hasattr(model.language_model, 'model'):
                model.language_model.model.resize_token_embeddings(target_vocab)
            else:

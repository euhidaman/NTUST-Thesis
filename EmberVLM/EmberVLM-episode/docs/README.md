...
- **[Stage 2: Multimodal Instruction Tuning](training/STAGE2_GUIDE.md)**
  ...
  - Knowledge distillation from SmolVLM
  - **NEW**: Multi-scale vision features (+3-5% improvement)
  - **NEW**: EMA self-distillation (no external teacher needed, +2-4%)
  - **NEW**: Layer-wise learning rates (+5-10% improvement)
...
│   300K samples  │  • EMA self-distillation (NEW)
│                 │  • Multi-scale vision (NEW)
│                 │  • Layer-wise LR (NEW)
└────────┬────────┘
         │
         ▼
...
```bash
# Train all 4 stages sequentially with improvements
...
- **Lightweight**: 135M total parameters (~35M trainable)
- **Efficient**: MobileViT-XS vision encoder + SmolLM-135M language model
- **Improved Training**: Multi-scale vision, EMA self-distillation, layer-wise LR

...
def plot_embedding_alignment(
    ...
    **Paper purpose**: Visualises how well image–text pairs are
    ...
    # Place legend outside the plot to avoid overlap with dense scatter data.
    # With 5 robot categories × 2 modalities = 10 entries, an interior legend
    # always covers data points.  Placing it below in two rows keeps it tidy.
    ax.legend(
        loc="upper center",
        bbox_to_anchor=(0.5, -0.10),
        ncol=min(5, len(ax.get_legend_handles_labels()[1]) // 2 or 1),
        fontsize=7,
        framealpha=0.9,
        markerscale=1.8,
        columnspacing=1.0,

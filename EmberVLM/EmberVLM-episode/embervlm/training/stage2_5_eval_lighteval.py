...
def run_stage2_5_evaluation(
    ...
    # ── VA Neuron Activation Heatmap (present vs absent objects) ──────────
    # Runs while the model is still loaded.  Works in both trial and main.
    _va_heatmap_path = None
    try:
        from embervlm.monitoring.va_neuron_analysis import generate_va_neuron_heatmap

        if checker.model is not None and checker.tokenizer is not None:
            # Grab evaluation images (same source the coherence tests used)
            va_images = get_sample_images_from_unibench(num_images=3)
            if va_images:
                _va_heatmap_path = generate_va_neuron_heatmap(
                    model=checker.model,
                    tokenizer=checker.tokenizer,
                    images=va_images,
                    output_dir=str(eval_dir / 'visualizations'),
                    device=checker.device,
                    dtype=checker.dtype,
                    preprocess_fn=checker._preprocess_image,
                )
                if _va_heatmap_path:
                    logger.info(f"✓ VA neuron activation heatmap saved: {_va_heatmap_path}")
            else:
                logger.info("VA neuron heatmap skipped: no evaluation images available")
    except Exception as e:
    ...
    # Attach VA heatmap path if it was generated earlier
    if _va_heatmap_path:
        results_summary['va_neuron_heatmap'] = _va_heatmap_path
